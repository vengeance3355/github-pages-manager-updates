...
function Invoke-WithRetry($label, [scriptblock]$action, $attempts = 16, $delayMs = 750) {
    $lastError = ""
...
function Start-UpdatedTarget {
    ...
    Start-Process -FilePath $Target -WorkingDirectory $targetDir | Out-Null
}
...
'@
    ...
    Start-Process powershell -WindowStyle Hidden -ArgumentList @(
        ...
        "-ExpectedSize", $expectedSizeValue,
        "-TargetVersion", ([string]$manifest.version),
        "-DiagPath", $UpdateApplyDiagPath
    )
