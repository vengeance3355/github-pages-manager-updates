...
try {
    ...
    try {
        ...
        Write-UpdateDiagnostic "ok" "Guncelleme uygulandi fakat BAT otomatik yeniden acilamadi. Dosyayi elle acabilirsin." @("restart failed: $($_.Exception.Message)")
    }
...
'@
    ...
    Write-UpdateApplyDiagnostic "launching" "Guncelleme supervisor baslatiliyor." @("primary=powershell", "fallback=cmd") $downloadPath $targetPath $backupPath ([string]$manifest.version) $expectedHash $expectedSizeValue
